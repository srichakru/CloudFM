...
import trapd_publishToPubSub as pubsub
...
def post_dmaap():
    ...
    ecomp_logger(tds.LOG_TYPE_DEBUG, tds.SEV_INFO, tds.CODE_GENERAL, msg)
    dataJson = json.dumps(post_data_enclosed)
    encoded_data = base64.b64encode(dataJson.encode()).decode()
    data_enclosed = {  "messages": [    {      "data": encoded_data,      "attributes": {}    }  ]}
    json_data = json.dumps(data_enclosed)
    k = 0
    ...
    while not dmaap_pub_success and k < (int(tds.c_config["publisher"]["http_retries"])):
        try:
            if (
               ...
               """
                #http_headers={"Authorization" : "Bearer " + auth_token,"Content-Type": "application/json"}
                http_headers={"Authorization" : "Bearer $(gcloud auth print-access-token)","Content-Type": "application/json"}
                http_resp = tds.http_requ_session.post(
                   "https://pubsub.googleapis.com/v1/projects/mongo-db-382501/topics/test-topic:publish",
                   json_data,
                   headers=http_headers,
                   timeout=tds.timeout_seconds,
                )
